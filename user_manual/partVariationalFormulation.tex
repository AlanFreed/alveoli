...
\end{equation}
where $n$ stands for number of Gauss points, and $\mathrm{w}_i$ and $\mathrm{w}_j$ denote the natural weight of the element.
...
\end{equation}
wherein $|\mathbf{J}|$ being the determinant of the Jacobian matrix in a tetrahedron that are calculated using derivatives of shape functions with respect to the local coordinates $(\xi, \eta, \zeta)$, and the current global coordinates $(x, y, z)$ via
\begin{equation}
\begin{aligned}
 \mathbf{J}= &
\begin{bmatrix}
...
\subsection{Stiffness Matrix for alveolar volume}
...
[\mathbf{h}_1], & [\mathbf{h}_2], & [\mathbf{h}_3], & [\mathbf{h}_4] 
\end{bmatrix}  \begin{Bmatrix} \boldsymbol{\Delta} \end{Bmatrix}  
= [\mathbf{H}_3] \begin{Bmatrix} \boldsymbol{\Delta} \end{Bmatrix} 
\end{equation}
where 
\begin{equation}
\mathbf{h}_i = \begin{bmatrix}
...
\subsection{Force Vector of Chord}
...
\begin{equation}
\mathbf{F}_{2D} = \int_{A} \mathbf{N}^{\mathsf{T}} \, \mathbf{t} \, \mathrm{d} A = \int_{\pentagon} \int_{\pentagon} \mathbf{N}^{\mathsf{T}} \, \mathbf{t} \,|\mathbf{J}| \, \mathrm{d} \xi \, \mathrm{d} \eta = \sum_{i=1}^{n} \sum_{j=1}^{n} \mathbf{N}^{\mathsf{T}} \, \mathbf{t} \,|\mathbf{J}| \, \mathrm{w}_i \mathrm{w}_j
\end{equation}
...
\begin{equation}
\mathbf{F}_{3D} = \int_{V} \mathbf{N}^{\mathsf{T}} \, \mathbf{t} \, \mathrm{d} V = \int \int \int \mathbf{N}^{\mathsf{T}} \, \mathbf{t} \,|\mathbf{J}| \, \mathrm{d} \xi \, \mathrm{d} \eta \, \mathrm{d} \zeta = \sum_{i=1}^{n} \sum_{j=1}^{n} \sum_{k=1}^{n} \mathbf{N}^{\mathsf{T}} \, \mathbf{t} \,|\mathbf{J}| \, \mathrm{w}_i \mathrm{w}_j \mathrm{w}_k
\end{equation}
wherein $|\mathbf{J}|$ being the determinant of the Jacobian matrix Eq.~(\ref{jacobiantet}) for tetrahedron, $n$ stands for number of Gauss points, $\mathrm{w}_i$ denotes the natural weight of the element, and $\mathbf{t}$ is the surface traction on the tetrahedron that is described in part 4.
