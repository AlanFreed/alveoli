...
def run():
    ...
    for i in range(steps):
        ...
        # far-field pure shear motion
        piF0[0, 0] += maxSqueeze / steps
        piF0[1, 1] = 1.0 / piF0[0, 0]
        pureShear[i] = log((piF0[0, 0] / piF0[1, 1])**(1.0/3.0))
        d.update(piF0)
        d.advance(pi)
    ...
    plt.xlabel(r'Far-Field $\ln \sqrt{a/b}$', fontsize=16)
    ...
    plt.title(r"Directions 1 \& 2", fontsize=20)
    plt.legend([line1, line2, line3],
               ["pentagon 1", "pentagon 2", "pentagon 5"],
               bbox_to_anchor=(0.025, 0.975), loc=2, fontsize=14)
    plt.xlabel(r'Far-Field $\ln \sqrt{a/b}$', fontsize=16)
    ...
    plt.savefig('pentagonalPureShear12.jpg')
